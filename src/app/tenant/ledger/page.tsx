...
import { ledgerBalance, ledgerSignedAmount, ledgerAttentionLabel, paymentPlanStatusLabel, agingBucket, installmentStatusLabel } from "@/lib/ledger";
...
export default async function TenantLedgerPage() {
  const user = await requireRole(["TENANT"], "/tenant/ledger");
  ...
  return (
    <main id="main-content" className="mx-auto max-w-6xl px-3 py-6 sm:px-4 lg:px-6">
      <div className="mb-4 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
        <p className="text-xs font-black uppercase tracking-[0.22em] text-brand-700">Resident ledger</p>
        <div className="mt-1 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div><h1 className="text-3xl font-black text-slate-950">Rent ledger</h1><p className="mt-1 max-w-3xl text-sm text-slate-600">Review rent charges, payments, credits, adjustments, and payment-plan activity connected to your tenancy.</p></div>
          <div className="flex flex-wrap gap-2"><Link className="inline-flex rounded-xl bg-brand-600 px-3 py-2 text-xs font-black text-white" href="/tenant/payments">Rent Center</Link></div>
        </div>
      </div>
      ...
      {plans.length > 0 ? (
        <section className="mb-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-500">Review installment plan status and upcoming due dates.</p>
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            {plans.map((plan) => {
              const nextInstallment = plan.installments.find((item) => item.status === "DUE" || item.status === "MISSED");
              ...
              return (
                <div key={plan.id} className="rounded-2xl border border-slate-200 p-4">
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    ...
                    <div className="rounded-xl bg-slate-50 p-3"><p className="text-xs font-bold uppercase text-slate-500">Next due</p><p className="mt-1 text-xl font-black text-slate-950">{nextInstallment ? formatCurrency(nextInstallment.amount) : "Complete"}</p></div>
                  </div>
                  <p className="mt-3 text-sm text-slate-600">{nextInstallment ? `${nextInstallment.dueDate.toLocaleDateString()} - ${agingBucket(nextInstallment.dueDate)} - ${installmentStatusLabel(nextInstallment.status)}` : "No open installments remain."}</p>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid gap-3">
        ...
        {entries.length === 0 ? <div className="rounded-3xl border border-slate-200 bg-white p-10 text-center text-slate-500 shadow-sm">No rent ledger activity is connected to your account yet.</div> : null}
      </div>
    </main>
  );
}
