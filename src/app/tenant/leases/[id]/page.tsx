...
export default async function TenantLeaseDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main id="main-content" className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="mb-6 rounded-3xl bg-slate-950 p-8 text-white shadow-sm">
        <p className="text-sm font-bold uppercase tracking-[0.25em] text-brand-200">Resident lease review</p>
        ...
        <p className="mt-3 text-slate-300">Lease packet status: {label(packet.status)}</p>
      </div>

      <section className="mb-6 grid gap-3 md:grid-cols-3">
        <Info label="Your signature" value={request ? label(request.status) : "Not assigned"} />
        <Info label="Packet progress" value={`${signedCount}/${packet.signatureRequests.length} signed`} />
        <Info label="Final lease" value={finalDocument ? "Ready" : "Pending"} />
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <aside className="space-y-6">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            {!request ? <p className="mt-3 text-sm text-slate-600">No resident signature request is assigned to you.</p> : isExpired ? (
              <div className="mt-4 rounded-2xl bg-amber-50 p-4 text-amber-900">
                <p className="text-sm font-bold uppercase">Expired</p>
                <p className="mt-2 text-sm">This signature request expired on {request.expiresAt?.toLocaleDateString()}. Contact your landlord for a new request.</p>
              </div>
              ...
              <form action={signApplicantLease} className="mt-5 space-y-4">
                ...
                <p className="text-xs leading-5 text-slate-500">HomeBase MLS stores the consent text, typed signature, timestamp, IP address, user agent, lease text hash, and signature evidence hash with this signature.</p>
          ...
          <Link href="/tenant/leases" className="inline-flex w-full justify-center rounded-2xl border border-slate-300 px-5 py-3 font-bold text-slate-900 hover:bg-slate-50">Back to leases</Link>
        </aside>
      </section>
    </main>
  );
...
function Info({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.2em] text-slate-500">{label}</p><p className="mt-2 text-lg font-black text-slate-950">{value}</p></div>;
}
