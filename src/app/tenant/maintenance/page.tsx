...
export default async function TenantMaintenancePage() {
  const user = await requireRole(["TENANT"], "/tenant/maintenance");
  const [activeUnits, requests] = await Promise.all([
  ...
  return (
    <main id="main-content" className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="rounded-3xl bg-slate-950 p-8 text-white shadow-sm">
        <p className="text-sm font-bold uppercase tracking-wide text-brand-200">Resident maintenance</p>
        <h1 className="mt-3 text-4xl font-black tracking-tight">Maintenance requests</h1>
        <p className="mt-3 max-w-3xl text-lg leading-8 text-slate-300">Submit repair requests, track status, and keep updates attached to your home.</p>
      </div>
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-[420px_1fr]">
        <form action={createMaintenanceRequest} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <label className="mt-5 block text-sm font-bold text-slate-700">Your rental</label>
          <select name="unitId" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3" required>
          ...
          {activeUnits.length === 0 ? <p className="mt-3 rounded-2xl bg-amber-50 p-3 text-xs font-bold text-amber-800">Maintenance requests require an active tenancy. Past requests still appear on the right.</p> : null}
        ...
        <div className="space-y-4">
          <h2 className="text-2xl font-black text-slate-950">Your maintenance history</h2>
          {requests.length === 0 ? (
