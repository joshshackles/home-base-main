...
export default async function ApplicantInboxPage() {
  const user = await requireRole(["APPLICANT", "TENANT"], "/applicant/inbox");
  const where = { OR: [{ createdById: user.userId }, { application: { OR: [{ applicantUserId: user.userId }, { applicantEmail: user.email }] } }, { maintenanceRequest: { requesterId: user.userId } }] };
