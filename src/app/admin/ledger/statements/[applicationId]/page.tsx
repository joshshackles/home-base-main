...
export default async function PrintableStatementPage({ params }: { params: { applicationId: string } }) {
  const application = await prisma.application.findUnique({
    where: { id: params.applicationId },
    include: {
      unit: { include: { property: true } },
      applicantUser: true,
      ledgerEntries: { where: { status: { not: LedgerEntryStatus.VOIDED } }, orderBy: [{ postedAt: "asc" }, { createdAt: "asc" }] },
      paymentPlans: { include: { installments: { orderBy: { dueDate: "asc" } } } }
    }
  });
  if (!application) notFound();

  const balance = ledgerBalance(application.ledgerEntries);
  const charges = application.ledgerEntries.filter((entry) => entry.type === "CHARGE" || entry.type === "ADJUSTMENT").reduce((sum, entry) => sum + entry.amount, 0);
  const payments = application.ledgerEntries.filter((entry) => entry.type === "PAYMENT" || entry.type === "CREDIT").reduce((sum, entry) => sum + entry.amount, 0);
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm print:border-0 print:shadow-none">
        <div className="flex flex-col gap-6 border-b border-slate-200 pb-6 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-4xl font-black text-slate-950">Ledger Statement</h1>
        ...
        <div className="grid gap-6 border-b border-slate-200 py-6 md:grid-cols-2">
          <div>
            ...
            <p className="mt-2 font-bold text-slate-900">{application.applicantName}</p>
          ...
          <div>
            <h2 className="text-lg font-black text-slate-950">Unit</h2>
            <p className="mt-2 font-bold text-slate-900">{application.unit.property.name} Unit {application.unit.unitNumber}</p>
            <p className="text-sm text-slate-600">{application.unit.property.addressLine}</p>
            <p className="text-sm text-slate-600">{application.unit.property.city}, {application.unit.property.state} {application.unit.property.zip}</p>
          </div>
        </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {application.ledgerEntries.map((entry) => (
              <tr key={entry.id}>
                ...
                <td className="px-3 py-3 text-slate-800">{entry.description}<br/><span className="text-xs text-slate-500">{entry.memo}</span></td>
            ...
            {application.ledgerEntries.length === 0 ? <tr><td colSpan={5} className="px-3 py-10 text-center text-slate-500">No ledger entries on this statement.</td></tr> : null}
          </tbody>
        </table>

        {application.paymentPlans.length > 0 ? (
            ...
            <div className="mt-3 grid gap-3">
              {application.paymentPlans.map((plan) => (
                <div key={plan.id} className="rounded-2xl border border-slate-200 p-4">
                  ...
                  <p className="text-sm text-slate-600">Status: {plan.status} · Total: {formatCurrency(plan.totalAmount)} · Installment: {formatCurrency(plan.installmentAmount)}</p>
                </div>
              ))}
            </div>
          </section>
