  ...
  const uploadPanel = (
    <form action={uploadLandlordDocument} encType="multipart/form-data" className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-4 space-y-3">
        ...
        <select name="leasePacketId" className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-bold" defaultValue=""><option value="">No lease packet</option>{leasePackets.map((packet) => <option key={packet.id} value={packet.id}>{packet.title} · {packet.application.applicantName}</option>)}</select>
