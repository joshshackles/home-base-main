...
import { MaintenancePriority, MessageThreadType, RentalLifecycleStatus } from "@prisma/client";
...
import { lifecycleLabel, lifecycleToUnitStatus, rentalLifecycleEngineSteps } from "@/lib/rental-lifecycle-engine";
...
export default async function LandlordUnitDetailPage({ params, searchParams }: { params: { id: string }; searchParams?: PageSearchParams }) {
  ...
  return (
    <main id="main-content" className="mx-auto max-w-[1500px] px-4 py-8 sm:px-6 lg:px-8">
      <LandlordPageHeader title={`Unit Workspace: ${unit.property.name} #${unit.unitNumber}`} description={`${unit.property.addressLine}, ${unit.property.city}, ${unit.property.state} ${unit.property.zip}`} actionHref="/landlord/inventory" actionLabel="Back to Inventory" />
      ...
      <nav aria-label="Unit workspace sections" className="sticky top-[73px] z-20 mt-6 flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white/95 p-2 shadow-sm backdrop-blur">
        {[
          ...
          <Link key={href} href={href} className="shrink-0 rounded-xl px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-100">
            {itemLabel}
          </Link>
        ))}
      </nav>
