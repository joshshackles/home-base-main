...
export default async function LandlordDashboardPage() {
  ...
  const [propertyCount, unitCount, availableCount, leadCount, applicationCount, maintenanceCount, taskCount, inboxCount, accessRequests] = await Promise.all([
    ...
    prisma.taskItem.count({ where: { OR: [{ createdById: user.userId }, { assignedToId: user.userId }, { unit: scope }, { property: { ownerId: user.userId, isArchived: false } }], status: { in: ["TODO", "IN_PROGRESS", "BLOCKED", "WAITING"] } } }),
  ...
  return (
    <WorkhorseDashboard
      ...
      tools={[
        ...
        { title: "Reports", detail: "Portfolio revenue, occupancy, leasing, maintenance, and communications reporting with exports.", href: "/landlord/reports", icon: dashboardIcons.applications }
      ]}
