      ...
      <div className="overflow-x-auto rounded-3xl border border-slate-200 bg-white shadow-sm">
        <table className="w-full min-w-[1120px] border-collapse text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-200">
            {leads.length === 0 ? (
              <tr><td colSpan={8} className="px-5 py-10 text-center text-slate-600">No leads have been submitted for your units yet.</td></tr>
            ) : leads.map((lead) => (
              <tr key={lead.id} className="align-top hover:bg-slate-50">
                ...
                <td className="px-5 py-4"><Link href={`/landlord/leads/${lead.id}`} className="rounded-xl bg-brand-600 px-4 py-2 text-xs font-bold text-white hover:bg-brand-700">Open</Link></td>
              </tr>
            ))}
          </tbody>
        </table>
