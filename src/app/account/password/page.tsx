...
export default async function PasswordPage({ searchParams }: { searchParams?: { error?: string; success?: string; reason?: string; retry?: string } }) {
  ...
  return (
    <main id="main-content" className="mx-auto max-w-xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <form key={searchParams?.retry || searchParams?.error || "password-form"} action={changePasswordAction} className="mt-6 space-y-4">
          <input type="hidden" name="required" value={required ? "1" : "0"} />
          ...
          <button type="submit" className="w-full rounded-2xl bg-brand-600 px-5 py-3 font-bold text-white hover:bg-brand-700">Update Password</button>
        </form>
      </div>
    </main>
  );
