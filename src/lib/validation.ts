...
export const applicantProfileSchema = z.object({
  ...
  driversLicenseState: z.string().trim().max(2, "Driver license state must use the two-letter abbreviation.").transform((value) => value ? value.toUpperCase() : null).nullable().optional(),
  ...
  licensePlateNumber: optionalText,
  licensePlateState: z.string().trim().max(2, "License plate state must use the two-letter abbreviation.").transform((value) => value ? value.toUpperCase() : null).nullable().optional(),
